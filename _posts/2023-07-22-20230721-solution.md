...
![Github stars](https://img.shields.io/github/stars/Panda-Ghost/Panda-Ghost.GitHub.io.svg?style=social&label=My Blog Stars)
...
$$
f_{a, i} =1 \space (i \in [0, n], a \le 0)\\
...
求 $\{a_i\}$ 的一个排列使 $\max_{1\le i\le n} \{ \sum_{j=1}^{i-1} a_j.w -a_i.s\}$ 最小，输出最小值即可
...
![一个环](../pic/20230721solgraph1.png)

如图中 4, 2 两点有 <4, 5, 1, 2> 和 <4, 3, 2> 两条路径

而考虑最小生成树的过程

![环的最小生成树](../pic/20230721solgraph2.png)
...
![5 到 1上面那条路径](../pic/20230721solgraph3.png)

所以只要 $L-4, L-6, L-8, ...$  中有一个为 $0$ 即需要提供原材料

等价于 $L\in \{x|x=2k,x\ge E,k\in \Z\}$ ，其中 $E$ 表示这条路径边数

若路径长为奇数

![5到1路径长为奇数](../pic/20230721solgraph4.png)

等价于 $L\in \{x|x=2k+1,x\ge E,k\in \Z\}$ ，其中 $E$ 表示这条路径边数
